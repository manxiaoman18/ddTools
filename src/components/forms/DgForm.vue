...
const funcChangeType = (jsonObj: JsonListItem) => {
  ...
  if (['checkbox', 'select'].includes(jsonObj.type || '')) {
  ...
  if (['checkbox', 'select'].includes(jsonObj.type || '')) {
    ...
    jsonObj.optionList?.forEach(item => {
      ...
      if (jsonObj.optionValueType) {
        if (!jsonObj.options) return
        jsonObj.options[item.key] = !isNaN(newVal) ? newVal : item.value
      } else {
  ...
  <div v-for="(jsonObj, index) in items" :key="index">
    <div class="form">
      <div class="form-item">
        ...
        <n-input v-model:value="jsonObj.key" disabled />
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">label:</label></template>
          入参字段名的别名，方便在dg上知道正在编辑的是什么参数
        </n-tooltip>
        <n-input v-model:value="jsonObj.label" />
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">placeholder:</label></template>
          入参字段在dg上的输入框为空时，在dg上显示的提示文字，一般写上该字段的含义，方便用户理解
        </n-tooltip>
        <n-input v-model:value="jsonObj.placeholder" />
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">type:</label></template>
          入参字段在dg上的输入框类型，有input、select、switch、checkbox、date五种，一般根据字段传参类型按需选择
        </n-tooltip>
        <n-radio-group class="right-ele" v-model:value="jsonObj.type" @change="funcChangeType(jsonObj)" name="radiogroup">
          <n-space>
      ...
      <div class="form-item" v-if="jsonObj.type === 'date'">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">format:</label></template>
          type字段为date时才显示该字段，控制当前字段传的时间参数的格式化规则，默认yyyy-MM-dd HH:mm:ss，格式化的字母大小写敏感
        </n-tooltip>
        <n-input v-model:value="jsonObj.format" />
      </div>
      <div class="form-item" v-if="jsonObj.type === 'date'">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">showTime:</label></template>
        ...
        <n-switch v-model:value="jsonObj.showTime" :rail-style="railStyle">
          <template #checked>精确到时分秒</template>
          <template #unchecked>精确到年月日</template>
        </n-switch>
      </div>
      <div class="form-item">
        ...
        <n-switch v-if="['checkbox', 'select'].includes(jsonObj.type || '')"
          v-model:value="jsonObj.optionValueType" :rail-style="railStyle"
          @update:value="updateOption(jsonObj)">
          <template #checked>number</template>
          <template #unchecked>string</template>
        </n-switch>
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">expand:</label></template>
        ...
        <n-switch v-model:value="jsonObj.expand" :rail-style="railStyle" @update:value="() => {
          ...
          <template #checked>默认折叠</template>
          <template #unchecked>默认露出</template>
        </n-switch>
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">required:</label></template>
          当前字段在dg页面是否必填
        </n-tooltip>
        <n-switch v-model:value="jsonObj.required" :rail-style="railStyle">
          <template #checked>字段必填</template>
          <template #unchecked>字段选填</template>
        </n-switch>
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">description:</label></template>
          显示在字段下方，可以用来描述字段意思和作用
        </n-tooltip>
        <n-input v-model:value="jsonObj.description" />
      </div>
      <div class="form-item">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">requiredMessage:</label></template>
          必填字段为空时的提示
        </n-tooltip>
        <n-input v-model:value="jsonObj.requiredMessage" />
      </div>
      <div class="form-item" title="默认值，type不同，默认值类型也不同，具体清参考dg文档">
        <n-tooltip trigger="hover" :show-arrow="false">
          <template #trigger><label class="left-ele">default:</label></template>
          字段在dg上显示的默认值，不修改的话会默认传该字段值，type不同，默认值类型也不同，具体清参照dg文档
        </n-tooltip>
        <template v-if="jsonObj.type === 'input'">
          <n-input v-model:value="jsonObj.default" class="right-ele" />
        </template>
        <template v-else-if="jsonObj.type === 'switch'">
          <n-switch v-model:value="jsonObj.default" />
        </template>
        <template v-else-if="jsonObj.type === 'select'">
          <n-radio-group class="right-ele" v-model:value="jsonObj.default" name="defaultgroup">
            <n-space v-for="item in jsonObj.optionList">
              <n-radio :value="!jsonObj.optionValueType ? item.value : parseFloat(item.value)">{{ item.key }}</n-radio>
            </n-space>
          </n-radio-group>
        </template>
        <template v-else-if="jsonObj.type === 'checkbox'" class="right-ele">
          <n-checkbox-group v-model:value="jsonObj.default">
            <n-space v-for="item in jsonObj.optionList">
              <n-checkbox :value="!jsonObj.optionValueType ? item.value : parseFloat(item.value)" :label="item.key" />
            </n-space>
          </n-checkbox-group>
        </template>
